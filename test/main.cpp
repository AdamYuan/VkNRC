...
std::vector<half> Evaluate(std::span<half> weights, std::span<half> inputs) {
	...
	for (int i = 0; i < 5; ++i) {
		...
		ActivateMatrix next_act_matrix =
		    (weight_matrix * act_matrix).unaryExpr([](const half &x) -> half { return half_float::fmax(x, half{0}); });
...
std::vector<float> Train(std::span<half> weights, std::span<half> inputs, std::span<half> targets) {
	if (inputs.size() / 64 != targets.size() / 4) {
		printf("Invalid\n");
		return {};
	}
	using ActivateMatrix = Eigen::Matrix<half, Eigen::Dynamic, Eigen::Dynamic, Eigen::ColMajor>;
	...
	weight_matrices.emplace_back(
	    Eigen::Map<WeightMatrix>(weights.data() + 5 * 64 * 64, (Eigen::Index)4, (Eigen::Index)64));
	...
	for (int i = 1; i < 7; ++i) {
		act_matrices.emplace_back((weight_matrices[i - 1] * act_matrices[i - 1]).unaryExpr([](const half &x) -> half {
			return half_float::fmax(x, half{0});
		}));
	}
	ActivateMatrix target_matrix =
	    Eigen::Map<WeightMatrix>(targets.data(), (Eigen::Index)4, (Eigen::Index)(targets.size() / 4));
	...
	std::vector<float> dw(5 * 64 * 64 + 4 * 64);
...
void test_train(std::size_t blocks) {
	std::mt19937 random{std::random_device{}()};
	std::vector<half> weights(64 * 64 * 5 + 64 * 4), inputs(kWorkgroupSize * blocks * 64),
	    targets(kWorkgroupSize * blocks * 4);

	for (int x = 0; auto &w : weights)
		w = std::uniform_real_distribution<float>{-0.02, 0.02}(random);
